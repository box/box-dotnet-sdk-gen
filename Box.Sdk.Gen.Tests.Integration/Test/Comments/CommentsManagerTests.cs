...
namespace Box.Sdk.Gen.Tests.Integration {
    ...
    public class CommentsManagerTests {
        ...
        public async System.Threading.Tasks.Task Comments() {
            ...
            Files uploadedFiles = await client.Uploads.UploadFileAsync(requestBody: new UploadFileRequestBody(attributes: new UploadFileRequestBodyAttributesField(name: fileName, parent: new UploadFileRequestBodyAttributesParentField(id: parentId)), file: fileByteStream)).ConfigureAwait(false);
            ...
            CommentFull newComment = await client.Comments.CreateCommentAsync(requestBody: new CreateCommentRequestBody(message: message, item: new CreateCommentRequestBodyItemField(id: fileId, type: CreateCommentRequestBodyItemTypeField.File))).ConfigureAwait(false);
            ...
            CommentFull newReplyComment = await client.Comments.CreateCommentAsync(requestBody: new CreateCommentRequestBody(message: message, item: new CreateCommentRequestBodyItemField(id: NullableUtils.Unwrap(newComment.Id), type: CreateCommentRequestBodyItemTypeField.Comment))).ConfigureAwait(false);
            ...
            CommentFull receivedComment = await client.Comments.GetCommentByIdAsync(commentId: NullableUtils.Unwrap(newComment.Id)).ConfigureAwait(false);
            ...
            await Assert.That.IsExceptionAsync(async() => await client.Comments.GetCommentByIdAsync(commentId: NullableUtils.Unwrap(newComment.Id)).ConfigureAwait(false));
