...
namespace Box.Tests.Integration {
    ...
    public class TasksManagerTests {
        ...
        public async System.Threading.Tasks.Task TestCreateUpdateGetDeleteTask() {
            Files files = await client.Uploads.UploadFileAsync(requestBody: new UploadFileRequestBodyArg(attributes: new UploadFileRequestBodyArgAttributesField(name: Utils.GetUUID(), parent: new UploadFileRequestBodyArgAttributesFieldParentField(id: "0")), file: Utils.GenerateByteStream(size: 10))).ConfigureAwait(false);
            FileFull file = files.Entries![0];
            Task task = await client.Tasks.CreateTaskAsync(requestBody: new CreateTaskRequestBodyArg(item: new CreateTaskRequestBodyArgItemField() { Type = CreateTaskRequestBodyArgItemFieldTypeField.File, Id = file.Id }) { Message = "test message", DueAt = "2035-01-01T00:00:00Z", Action = CreateTaskRequestBodyArgActionField.Review, CompletionRule = CreateTaskRequestBodyArgCompletionRuleField.AllAssignees }).ConfigureAwait(false);
            Assert.IsTrue(task.Message == "test message");
            Assert.IsTrue(task.Item!.Id == file.Id);
            Task taskById = await client.Tasks.GetTaskByIdAsync(taskId: task.Id!).ConfigureAwait(false);
            ...
            Task updatedTask = await client.Tasks.UpdateTaskByIdAsync(taskId: task.Id!, requestBody: new UpdateTaskByIdRequestBodyArg() { Message = "updated message" }).ConfigureAwait(false);
