...
namespace Box.Tests.Integration {
    ...
    public class SessionTerminationManagerTests {
        ...
        public async System.Threading.Tasks.Task TestSessionTerminationUser() {
            ...
            SessionTerminationMessage result = await client.SessionTermination.CreateUserTerminateSessionAsync(requestBody: new CreateUserTerminateSessionRequestBodyArg(userIds: Array.AsReadOnly(new [] {Utils.GetEnvVar(name: "USER_ID")}), userLogins: Array.AsReadOnly(new [] {user.Login!}))).ConfigureAwait(false);
        ...
        public async System.Threading.Tasks.Task TestSessionTerminationGroup() {
            ...
            GroupFull group = await client.Groups.CreateGroupAsync(requestBody: new CreateGroupRequestBodyArg(name: groupName)).ConfigureAwait(false);
            SessionTerminationMessage result = await client.SessionTermination.CreateGroupTerminateSessionAsync(requestBody: new CreateGroupTerminateSessionRequestBodyArg(groupIds: Array.AsReadOnly(new [] {group.Id}))).ConfigureAwait(false);
            Assert.IsTrue(result.Message == "Request is successful, please check the admin events for the status of the job");
